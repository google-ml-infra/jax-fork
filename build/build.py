...
from tools import utils, command
...
def add_build_cuda_with_clang_argument(parser: argparse.ArgumentParser):
  parser.add_argument(
      ...
      action="store_true",
      help="Should we build CUDA code using Clang? The default value "
...
def add_requirements_nightly_update_argument(parser: argparse.ArgumentParser):
  parser.add_argument(
    "--nightly_update",
      action="store_true",
    help="If true, updates requirements_lock.txt for a corresponding version of "
...
async def main():
  ...
  requirements_update_parser = subparsers.add_parser("requirements_update", help="Updates the requirements_lock.txt files")
  ...
  if args.command == "requirements_update":
    ...
    if args.nightly_update:
      logging.debug("--nightly_update is set. Bazel will run //build:requirements_nightly.update")
  ...
  target_cpu = (wheel_cpus[args.target_cpu] if args.target_cpu is not None
              else arch)

  if args.ci_mode:
    logging.debug("Running in CI mode. Run the CLI with --help for more details on what this means.")
    bazelrc_config = utils.get_bazelrc_config(os_name, arch, args.command, args.use_rbe)
    ...
    if args.target_cpu_features == "release":
      ...
      if utils.is_windows():
        logger.warning("--target_cpu_features=native is not supported on Windows; ignoring.")
      else:
    ...
    if "cuda" in args.command:
      ...
      if args.build_cuda_with_clang:
        ...
        bazel_command.append(f"--action_env=CLANG_CUDA_COMPILER_PATH={clang_path}")
      else:
